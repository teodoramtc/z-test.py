...
p1 = st.number_input("Proportion in the first group:", min_value=float(0), max_value=float(100),
                     value=float(0), step=float(0.01))
p2 = st.number_input("Proportion in the second group:", min_value=float(0), max_value=float(100),
                     value=float(0), step=float(0.01))
...
z = round((p1 - p2) / np.sqrt((p_pooled*(1-p_pooled))*(1/n1 + 1/n2)), 2)
